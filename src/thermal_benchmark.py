...
from lame_penguin import LamePenguin
...
def main(config_file: str, log_level: int) -> int:
    ...
    # Image dir for first test case
    image_dir = image_dir.joinpath(f"{config_file.stem}_single")
    # shutil.rmtree(image_dir, ignore_errors=True)
    image_dir.mkdir(mode=0o775, exist_ok=True)

    # Create environment for single penguin
    env = Environment(
        log_level,
        "Thermal Benchmark Single",
        image_dir,
        env_size,
    ...
    # Place one stationary penguin in the center
    penguin = LamePenguin(
        16,
        16,
    ...
    env.add_agent(penguin)
    env.run()

    # Image dir for second test case
    image_dir = image_dir.parent.joinpath(f"{config_file.stem}_group_close")
    # shutil.rmtree(image_dir, ignore_errors=True)
    image_dir.mkdir(mode=0o775, exist_ok=True)
    ...
    coords = [(13, 13), (13, 17), (17, 13), (17, 17)]
    for x, y in coords:
        penguin = LamePenguin(
            x,
            y,
